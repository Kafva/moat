...
use rocket::{
    data::ByteUnit, data::FromData, http::Status, request::FromRequest, Data,
    Request,
};
...
impl std::fmt::Display for KeyErr<'_> {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match *self {
            KeyErr::InvalidKey(key) => write!(f, "Invalid key '{}'", key),
            KeyErr::MissingKey(key) => write!(f, "Missing key '{}'", key),
...
fn get_value_for_key<'a, T: std::str::FromStr>(
    key: &'a str,
    body_as_string: &str,
    regex: Regex,
) -> Result<T, KeyErr<'a>> {
...
impl<'r> FromData<'r> for ReadToggleData {
    ...
    async fn from_data(
        ...
        let b64_rssurl_regex =
            Regex::new(r"rssurl=([=+/a-zA-Z0-9]{3,128})").unwrap();
        let unread_regex = Regex::new(r"unread=(true|false)").unwrap();

        let body_as_string =
            std::str::from_utf8(&buf).expect("Failed to decode request body");

        match get_value_for_key::<u32>("id", body_as_string, id_regex) {
            /***** `id` in params *******/
            Ok(id) => match get_value_for_key::<bool>(
                "unread",
                body_as_string,
                unread_regex,
            ) {
                Ok(unread) => data::Outcome::Success(ReadToggleData {
                    id: Some(id),
                    rssurl: None,
                    unread: unread,
                }),
            ...
            /***** `rssurl` in params *******/
            Err(_) => match get_value_for_key::<String>(
                "rssurl",
                body_as_string,
                b64_rssurl_regex,
            ) {
                Ok(b64_rssurl) => {
                    match base64::decode(b64_rssurl) {
                        Ok(decoded) => {
                            ...
                            data::Outcome::Success(ReadToggleData {
                                rssurl: Some(rssurl),
                                id: None,
    ...
    async fn from_request(
        ...
        match req.headers().get_one("x-creds") {
            None => request::Outcome::Failure((
                Status::Unauthorized,
                KeyErr::MissingKey("x-creds"),
            )),
            Some(key) if key == env_key => {
                request::Outcome::Success(Creds(key))
            }
            Some(_) => request::Outcome::Failure((
                Status::Unauthorized,
                KeyErr::InvalidKey("x-creds"),
            )),
        }
    }
}
