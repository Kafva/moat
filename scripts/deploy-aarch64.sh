...
helpStr="Build and deploy the server to a machine running aarch64 Arch Linux (requires an Arch Linux development environment)"
...
sed "s@SET WORK DIR@/home/jonas/Repos/moat@; s/CHANGE THIS/$(pass moat)/" ./conf/moat.service > /tmp/moat_$remote.service
...
if $(grep -q 'ID=arch' /etc/os-release 2> /dev/null ); then 
	...
	printf "=> Localhost is not running Arch Linux\n"
	printf "=> Compile directly on the target machine.\n"
fi
