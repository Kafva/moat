    ...
    var body: some View {
        HStack {
            ...
            VStack(alignment: .leading, spacing: 5) {
                ...
                Link(
                    "\(URL(string: feed.url)?.host ?? "???")",
                    destination: URL(string: feed.url)!
                )
            ...
            if !self.feed.muted {
                Text("\(self.unread_count)/\(feed.item_count)")
                    ...
                    .frame(
                        ...
                        width: (self.showLogos
                            ? self.screenWidth * 0.5 : self.screenWidth * 0.4)
                            - (IMAGE_WIDTH + 5),
                    ...
                    .onTapGesture {
                        self.alertState.title =
                            "Mark all entries for \(self.feed.title) as read?"
        ...
        // in case the '/update' endpoint returns an error
